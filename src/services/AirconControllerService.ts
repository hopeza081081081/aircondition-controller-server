...
import { MQTT_QOS, MQTT_TOPICS } from "../config/constants";
const Logger = require("../utils/Logger");

export class AirconControllerService {
   ...
   */
  public async setAllControllers(
    command: boolean,
    reason: string,
  ): Promise<void> {
   ...
   * @param reason - Reason for the command (for logging)
   */
  public async setSingleController(
    controllerId: number,
    command: boolean,
    reason: string,
  ): Promise<void> {
    ...
    try {
      ...
      if (this.localMqtt && this.localMqtt.isConnected()) {
        ...
        Logger.warn(
          `Local MQTT not connected - skipping controller ${controllerId} ${command ? "on" : "off"} command`,
        );
      }
      ...
      Logger.debug(
        `Controller ${controllerId} set to ${command ? "on" : "off"}`,
        { reason },
      );
    } catch (error) {
      Logger.error(
        `Failed to set controller ${controllerId} to ${command ? "on" : "off"}`,
        error as Error,
        { reason },
      );
    }
   ...
   */
  public async turnOffAll(reason: string): Promise<void> {
   ...
   */
  public async turnOnAll(reason: string): Promise<void> {
  ...
  private async _relayToCloud(
    ...
    try {
      ...
      Logger.warn(
        `Failed to relay controller ${controllerId} command to cloud`,
        cloudError as Error,
      );
